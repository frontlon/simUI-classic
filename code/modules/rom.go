...
func OpenRomPathFolder(platform uint32) error {
	...
	fmt.Println(folder)
	if err := components.OpenFolderByWindow(folder); err != nil {
		return err
	}
	return nil
}
...
// 添加游戏分身
func AddSlnkGame(platform uint32, menu string, gameType string, files []string, cmd string) error {
	...
	for _, f := range files {
		...
		if err := components.SaveSlnkFile(p, gameType, relPath, cmd); err != nil {
			return err
		}
	}
