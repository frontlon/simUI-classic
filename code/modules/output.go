...
func outputByPlatform(request *request.OutputRom) {
	...
	for _, opt := range request.Options {
		...
		for _, p := range files {
			...
			defer openF.Close()
			if err == nil {
				_ = compoments.CompressZip(openF, opt, zw)
			}
		}
	}
		...
		for _, simId := range request.Simulators {
			...
			if err := filepath.Walk(utils.GetFilePath(sim.Path),
				func(p string, f os.FileInfo, err error) error {
					...
					_ = compoments.CompressZip(fo, romOpt, zw)
	...
	if utils.InSliceString("subgame", request.Options) {
		...
		if err := compoments.CreateOutputIni(subgames); err != nil {
			return
		}
		f, err := os.Open(compoments.TmpOutputIniPath)
		defer func() {
			f.Close()
			utils.FileDelete(compoments.TmpOutputIniPath)
		}()
		if err == nil {
			_ = compoments.CompressZip(f, "", zw)
		}
...
func outputById(request *request.OutputRom, roms []*db.Rom) {
	...
	for _, opt := range request.Options {
		...
		//读取所有资源
		resMap, _ := compoments.GetAllRomThumbsToMap(resPath, romMap)
			...
			if _, ok := resMap[fileName]; ok {
				for _, sp := range resMap[fileName] {
					...
					if err == nil {
						...
						_ = compoments.CompressZip(f, romOpt, zw)
					}
				}
			}
		}
	...
	if err == nil {
		_ = compoments.CompressZip(csvf, "", zw)
	}
