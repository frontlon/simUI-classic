...
func RomController() {
	...
	//添加pc游戏
	utils.Window.DefineFunction("AddPcGame", func(args ...*sciter.Value) *sciter.Value {
		...
		d := []string{}
		_ = json.Unmarshal([]byte(datastr), &d)

		err := modules.AddPcGame(platform, menu, d)
		if err != nil {
			utils.WriteLog(err.Error())
			return utils.ErrorMsg(err.Error())
		}
	...
	utils.Window.DefineFunction("GetGameList", func(args ...*sciter.Value) *sciter.Value {
		...
		if data.BaseType == config.Cfg.Lang["BaseType"] {
			data.BaseType = ""
		}

		if data.BaseProducer == config.Cfg.Lang["BaseProducer"] {
			data.BaseProducer = ""
		}

		if data.BasePublisher == config.Cfg.Lang["BasePublisher"] {
			data.BasePublisher = ""
		}
		if data.BaseYear == config.Cfg.Lang["BaseYear"] {
			data.BaseYear = ""
		}
		if data.BaseCountry == config.Cfg.Lang["BaseCountry"] {
			data.BaseCountry = ""
		}
		if data.BaseTranslate == config.Cfg.Lang["BaseTranslate"] {
			data.BaseTranslate = ""
		}
		if data.BaseVersion == config.Cfg.Lang["BaseVersion"] {
			data.BaseVersion = ""
		}
		newlist := []*db.Rom{}
		if data.Num == "" {
			newlist, _ = (&db.Rom{}).Get(data.ShowSubGame, data.ShowHide, data.Page, data.Platform, data.Catname, data.Keyword, data.BaseType, data.BasePublisher, data.BaseYear, data.BaseCountry, data.BaseTranslate, data.BaseVersion, data.BaseProducer)
		} else {
			//按拼音查询
	...
	utils.Window.DefineFunction("GetGameCount", func(args ...*sciter.Value) *sciter.Value {
		data := &request.GetGameList{}
		_ = json.Unmarshal([]byte(args[0].String()), &data)
		count, _ := (&db.Rom{}).Count(data.ShowHide, data.Platform, data.Catname, data.Keyword, data.BaseType, data.BasePublisher, data.BaseYear, data.BaseCountry, data.BaseTranslate, data.BaseVersion, data.BaseProducer)
	...
	utils.Window.DefineFunction("DeleteRom", func(args ...*sciter.Value) *sciter.Value {
		...
		err = (&db.Rom{}).DeleteSubRom(info.Platform, info.FileMd5)
		if err != nil {
			utils.WriteLog(err.Error())
		}
