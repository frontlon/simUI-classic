...
function changeType(type) {
    ...
    $(#simulator_h2).style["display"] = "none";
    $(#simulator_ul).style["display"] = "none";
}
...
function changePlatform(platformId) {
    ...
    //生成模拟器列表
    if (CONF.Platform[platformId.toString()].SimList != undefined) {
        var html = "";
        var list = CONF.Platform[platformId.toString()].SimList;
        for (var sid in list) {
            html += "<li><label><input type='checkbox' value=" + list[sid].Id + " checked />" + list[sid].Name + "</label></li>";
        }
        $(#simulator_ul).html = html;
    }

    //显示dom
    $(#options_h2).style["display"] = "block";
    $(#options_ul).style["display"] = "block";
    $(#simulator_h2).style["display"] = "block";
    $(#simulator_ul).style["display"] = "block";
...
function outputData() {
    ...
    //模拟器
    var sims = $$(#simulator_ul input[type = 'checkbox']);
    for (var opt in sims) {
        if (opt.checked) {
            simulators.push(opt.value.toInteger());
        }
    }
