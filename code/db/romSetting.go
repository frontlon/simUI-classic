...
//更新rom 目录配置
func (m *RomSetting) UpdateMenu() error {

	//初始化数据
	_ = m.InitData(m.Platform, m.FileMd5)

	//更新数据
	result := getDb().Table(m.TableName()).Where("platform=? AND file_md5 = ?", m.Platform, m.FileMd5).Update("menu", m.Menu)
	if result.Error != nil {
		fmt.Println(result.Error)
		return result.Error
	}
...
//清除 rom 目录配置
func (m *RomSetting) ClearMenu() error {

	//检查记录是否存在
	vo := &RomSetting{}
	exists := getDb().Where("platform=? AND file_md5 = ?", m.Platform, m.FileMd5).First(&vo)
	if exists.Error != nil {
		return nil
	}

	//更新数据
	result := getDb().Table(m.TableName()).Where("platform=? AND file_md5 = ?", m.Platform, m.FileMd5).Update("menu", "")
	if result.Error != nil {
		fmt.Println(result.Error)
		return result.Error
	}
...
//更新通关状态
func (m *RomSetting) UpdateComplete() error {
	//初始化数据
	_ = m.InitData(m.Platform, m.FileMd5)

	result := getDb().Table(m.TableName()).Where("platform=? AND file_md5 = ?", m.Platform, m.FileMd5).Update("complete", m.Complete)
	if result.Error != nil {
		fmt.Println(result.Error)
	}
	return result.Error
...
//有空游戏统计信息
func (m *RomSetting) TruncateGameStat() error {
